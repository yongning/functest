...
eth1macaddr="$(LANG=C ifconfig enp4s0 | grep -Po 'HWaddr \K.*$')"
if [ $? -ne 0 ] ; then
    zenity --error --text="英特尔i210以太网网卡地址获取失败"
    exit 1
fi
eth2macaddr="$(LANG=C ifconfig enaftgm1i0 | grep -Po 'HWaddr \K.*$')"
if [ $? -ne 0 ] ; then
    zenity --error --text="飞腾内置以太网卡1网卡地址获取失败"
    exit 1
fi
...
#    "$REALDIR/i2c1cmd" -c 1 -a 1 -v aa
    ...
    temp="$("$REALDIR/i2c1cmd" -c 1 -a 33 -v 1)"
    echo $temp
    tmp1="$(echo $temp | grep "i2c1cmd_error" | wc -l)"
    echo $tmp1
    if [ $tmp1 -ne 0 ] ; then
